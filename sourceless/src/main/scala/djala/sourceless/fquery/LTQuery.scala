...
final case class InsertQueryLT(tableName: String, columns: List[DBAttr[_]], values: List[List[LTQuery]]) extends LTQuery

trait ValueLT extends LTQuery
final case class StringValueLT(v: String) extends ValueLT
final case class IntValueLT(i: Int) extends ValueLT
...
object LTQuery {
  def fromFQuery[A](fquery: FQuery[A]): LTQuery = {
    fquery match {
      ...
      case iq @ InsertQuery(values, table)  => InsertQueryLT(table.name, iq.shapeToList, iq.valuesToLTQuery)
    }
  }
}
